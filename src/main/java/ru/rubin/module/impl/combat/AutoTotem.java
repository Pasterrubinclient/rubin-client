...
import ru.rubin.util.other.StopWatchP;
...
public class AutoTotem extends Module {
   ...
   private final SliderSetting crystalDistance = new SliderSetting("Дистанция до кристалла", 4.0F, 1.0F, 10.0F, 1.0F, false)
   ...
   public AutoTotem() {
      this.addSettings(new Setting[]{this.options, this.health, this.elytraHealth, this.crystalDistance, this.tntDistance, this.noBall});
   }
   ...
   private boolean elytraCheck() {
      ...
      boolean elytra = chestStack.getItem() == Items.ELYTRA && this.options.get("Р—РґРѕСЂРѕРІСЊРµ СЃ СЌР»РёС‚СЂР°РјРё");
      return elytra && this.checkHealth();
   }

   private boolean checkFall() {
      return !this.options.get("РџР°РґРµРЅРёРµ") ? false : mc.player.fallDistance > 2.0;
   }
   ...
   private boolean checkCrystal() {
      if (!this.options.get("Р­РЅРґРµСЂ-РєСЂРёСЃС‚Р°Р»Р»")) {
         return false;
      } else {
   ...
   private boolean checkTnt() {
      if (!this.options.get("Р”РёРЅР°РјРёС‚")) {
         return false;
      } else {
         for (Entity entity : mc.world.getEntities()) {
            float distance = mc.player.distanceTo(entity);
            if ((entity instanceof TntEntity || entity instanceof TntMinecartEntity) && distance <= this.tntDistance.get()) {
               return true;
            }
         }
