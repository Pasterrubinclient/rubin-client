...
import ru.rubin.event.impl.EventChangeWorld;
import ru.rubin.event.impl.EventScreen;
...
public class NameTags extends Module {
   ...
   public BooleanSetting armor = new BooleanSetting("Отображать броню", true).hidden(() -> !this.entityType.get("Player"));
   public BooleanSetting showMainHand = new BooleanSetting("Правая рука", true).hidden(() -> !this.entityType.get("Player"));
   public BooleanSetting showOffHand = new BooleanSetting("Левая рука", true).hidden(() -> !this.entityType.get("Player"));
         ...
         for (PlayerEntity entity : entityPositions.keySet()) {
            if (entity != mc.player) {
               ...
               if (pos != null && !(pos[2] < 0.0) && !(pos[2] > 1.0)) {
                  ...
                  int color = Rubin.get.friendManager.isFriend(entity.getName().getString())
                  ...
                  boolean isFriend = Rubin.get.friendManager.isFriend(name);
                  String friendTag = isFriend ? "[F] " : "";
                  float fontSize = 28.0F;
                  float friendTagWidth = isFriend ? renderer.measureText(FontRegistry.INTER_MEDIUM, friendTag, fontSize).width : 0.0F;
                  ...
                  if (isFriend) {
                     renderer.text(FontRegistry.INTER_MEDIUM, textX, 4.75F, fontSize, "[F]", Renderer2D.ColorUtil.getColor(40, 255, 40, 255));
                     textX += friendTagWidth;
                  }

                  renderer.text(FontRegistry.INTER_MEDIUM, textX, 4.75F, fontSize, name, Renderer2D.ColorUtil.getTextColor(1, 1));
                  textX += nameWidth + spaceWidth;
                  renderer.text(FontRegistry.INTER_MEDIUM, textX, 4.75F, fontSize, hpText, Renderer2D.ColorUtil.getMainColor(1, 1));
                  renderer.popScale();
                  ...
                  if (this.armor.get()) {
                     ...
                     this.drawArmor(renderer, drawContext, entity, (float)pos[0] * 2.0F, (float)pos[1] * 2.0F, scale, nameRectWidth);
                     renderer.popScale();
                     renderer.popTransform();
                  }
   ...
   private void drawArmor(Renderer2D renderer, DrawContext drawContext, PlayerEntity player, float baseX, float baseY, float scale, float nameRectWidth) {
   ...
   private void drawPlayerItems(Renderer2D renderer, PlayerEntity player, float centerX, float startY, float fontHeight, int color) {
      List<Text> items = new ArrayList<>();
      if (this.showMainHand.get() && !player.getMainHandStack().isEmpty()) {
         items.add(player.getMainHandStack().getName());
      }

      if (this.showOffHand.get() && !player.getOffHandStack().isEmpty()) {
         items.add(player.getOffHandStack().getName());
      }

      float y = startY;

      for (Text item : items) {
         float w = renderer.measureText(FontRegistry.INTER_MEDIUM, item.getString(), fontHeight * 2.0F).width + 12.0F;
         Hud.drawClientRect(renderer, -w / 2.0F, y - 1.0F, w, fontHeight + 10.0F, 7.0F, 1.0F, 1.0F);
         renderer.text(FontRegistry.INTER_MEDIUM, -w / 2.0F + 6.0F, y + 15.0F, fontHeight * 2.0F, item.getString(), Renderer2D.ColorUtil.getTextColor(1, 1));
         y += fontHeight + 12.0F;
      }
   }
